        ...
        <div class="bg-white shadow-md rounded-lg p-8">
            <form action="{{ route('bands.update', $band) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="name" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" value="{{ $band->name }}" required>
                </div>

                <div class="grid grid-cols-2 gap-6 mb-6">
                    <div>
                        <label for="origin" class="block text-sm font-medium text-gray-700 mb-2">Origin</label>
                        <input type="text" name="origin" id="origin" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" value="{{ $band->origin }}">
                    </div>
                    <div>
                        <label for="genre" class="block text-sm font-medium text-gray-700 mb-2">Genre</label>
                        <input type="text" name="genre" id="genre" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" value="{{ $band->genre }}">
                    </div>
                </div>

                <div class="mb-6">
                    <label for="bio" class="block text-sm font-medium text-gray-700 mb-2">Biography</label>
                    <textarea name="bio" id="bio" rows="6" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ $band->bio }}</textarea>
                </div>
                ...
                <div class="mb-6">
                    ...
                    @if ($band->getFirstMediaUrl('band-photos'))
                        <img src="{{ $band->getFirstMediaUrl('band-photos') }}" alt="{{ $band->name }}" class="mt-4 h-32">
                    @endif
                </div>
