...
<header class="pb-6 border-b border-gray-200">
    ...
    <div class="mb-4">
        <a href="#" {{-- Ganti # dengan link ke halaman kategori --}}
            class="text-sm font-bold uppercase tracking-widest text-red-600 hover:text-red-800 transition-colors">
...
{{-- Konten Body Artikel --}}
<div class="mt-8">
    {{-- Class 'prose-red' menyesuaikan warna link/aksen di konten agar cocok dengan tema --}}
    {!! $article->body !!}
</div>
...
@if ($article->tags->isNotEmpty())
    <div class="mt-10 pt-6 border-t border-gray-200">
        <div class="flex flex-wrap items-center gap-3">
            <span class="font-semibold text-gray-800">Topik Terkait:</span>
            @foreach ($article->tags as $tag)
                <a href="#" {{-- Ganti # dengan link ke halaman tag --}}
                    class="bg-slate-100 text-slate-700 text-sm font-medium px-4 py-1.5 rounded-full transition-colors duration-300 hover:bg-red-100 hover:text-red-700">
                    #{{ $tag->name }}
                </a>
            @endforeach
        </div>
