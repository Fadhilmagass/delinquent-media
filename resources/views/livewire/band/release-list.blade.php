<div class="space-y-4">
    @forelse ($releases as $release)
        <a href="{{ route('releases.show', $release) }}"
            ...
            <div
                class="flex items-center bg-white dark:bg-slate-900/70 p-4 rounded-2xl shadow-lg border border-slate-100 dark:border-slate-800 h-full transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-2xl hover:border-sky-500/40">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex items-center text-sm text-slate-500 dark:text-slate-400 mt-1 space-x-2">
                        <span
                            class="font-semibold text-sky-600 dark:text-sky-400 px-2 py-0.5 bg-sky-100 dark:bg-sky-900/60 rounded-md shadow-sm">{{ $release->type->value }}</span>
                        <span>&bull;</span>
                        <span>{{ $release->release_date->format('Y') }}</span>
                    </div>
                </div>
                <div class="ml-4 text-slate-300 dark:text-slate-500 flex items-center">
    ...
    @empty
        <div
            class="text-center py-12 px-6 bg-white dark:bg-slate-900/70 rounded-2xl shadow-lg border border-slate-100 dark:border-slate-800">
            ...
            <h3 class="mt-2 text-lg font-semibold text-slate-800 dark:text-white">No Releases Yet</h3>
