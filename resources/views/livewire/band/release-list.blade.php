<div class="space-y-4">
    @forelse ($releases as $release)
        <a href="{{ route('releases.show', $release) }}"
            ...
            <div
                class="flex items-center bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-lg border border-slate-100 dark:border-slate-700 h-full transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-2xl hover:border-sky-200 dark:hover:border-sky-600">
                <div class="w-20 h-20 rounded-xl mr-5 overflow-hidden flex-shrink-0 shadow-lg relative">
                    ...
                    @else
                        <div
                            class="w-full h-full bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center">
                ...
                <div class="flex-1">
                    <p class="font-bold text-lg text-slate-800 dark:text-slate-200 truncate">{{ $release->title }}</p>
                    <div class="flex items-center text-sm text-slate-500 dark:text-slate-400 mt-1 space-x-2">
                        <span
                            class="font-semibold text-sky-600 dark:text-sky-300 px-2 py-0.5 bg-sky-100 dark:bg-sky-900/50 rounded-md shadow-sm">{{ $release->type->value }}</span>
                        <span>&bull;</span>
                        <span>{{ $release->release_date->format('Y') }}</span>
                    </div>
                </div>
                <div class="ml-4 text-slate-300 dark:text-slate-600 flex items-center">
    ...
    @empty
        <div class="text-center py-12 px-6 bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-100 dark:border-slate-700">
            ...
            <h3 class="mt-2 text-lg font-semibold text-slate-800 dark:text-slate-200">No Releases Yet</h3>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">This band hasn't released any music yet.</p>
        </div>
    @endforelse
