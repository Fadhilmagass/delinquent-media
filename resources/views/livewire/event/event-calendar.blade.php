<div class="bg-slate-50 dark:bg-slate-900 min-h-screen">
    <div class="container mx-auto px-4 py-8 md:py-12">
        ...
        <div class="flex justify-center mb-10">
            <div class="inline-flex rounded-lg shadow-md overflow-hidden border border-slate-200 dark:border-slate-700">
                <button type="button" wire:click="setViewMode('list')"
                    class="px-5 py-2 text-sm font-semibold focus:outline-none transition flex items-center gap-2 {{ $viewMode === 'list' ? 'bg-red-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' }}">
                ...
                <button type="button" wire:click="setViewMode('calendar')"
                    class="px-5 py-2 text-sm font-semibold focus:outline-none transition flex items-center gap-2 {{ $viewMode === 'calendar' ? 'bg-red-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @if ($events->count())
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($events as $event)
                            <div
                                class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-100 dark:border-slate-700 overflow-hidden group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                                <a href="{{ route('events.show', $event) }}" class="block">
                                    <div class="relative">
                                        ...
                                        <div
                                            class="absolute top-3 right-3 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm text-center rounded-lg px-3 py-2 shadow">
                                            <span
                                                class="text-2xl font-extrabold text-red-600 dark:text-sky-400 leading-none">{{ $event->event_time->format('d') }}</span>
                                            <span
                                                class="text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wide">{{ $event->event_time->format('M Y') }}</span>
                                        </div>
                                    </div>
                                    <div class="p-4">
